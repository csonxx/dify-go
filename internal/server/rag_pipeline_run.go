...
func ragPipelinePreviewOutputs(datasourceInfoList []map[string]any) map[string]any {
	totalNodes := len(datasourceInfoList)
	if totalNodes <= 0 {
		totalNodes = 1
	}
	...
	for i := 0; i < totalNodes; i++ {
		preview = append(preview, map[string]any{
			"content":      "Preview chunk " + strconv.Itoa(i+1) + " generated by dify-go rag pipeline compatibility layer.",
			"child_chunks": []string{},
